...
export const CinemasBody = () => {
    ...
    const theators = useSelector((store)=>store.theators.cinemas_data)
    console.log(theators)
                                    ...
                                    <div>
                                        {
                                            cinema.timings?.map((time, index) => {
                                                ...
                                                return (
                                                    <div onClick={() => {localStorage.setItem("theaterDet",JSON.stringify({name:cinema.name, time:time.time})),showModal()}} style={
                                                        amOrPm === "AM" || (showTime === currentTime ? showMinutes > new Date().getMinutes() ? true : false : (showTime > currentTime && showTime !== 12)) || (new Date().getDate()+1 > new Date().getDate()) ? { pointerEvents: "all" } :
