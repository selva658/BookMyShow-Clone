...
var mm = String(todays.getMonth() + 1).padStart(2, '0'); //January is 0!
  ...
  const handleChange = () => {};
  const bookedData = JSON.parse(localStorage.getItem("BookedMovie"))
  return (
    <div>
      <div className="left">
      ...
      <div className="right">
        <div className="summeryPart">
          ...
          <div className="categories">
            <div style={{ textTransform: "uppercase" }}>{bookedData.MovieName}</div>
            {/* <div>{booking_details.silver.length + booking_details.platinium.length} Ticket(s)</div> */}
          </div>
          <span>{bookedData.theater}</span>
          <br></br>
          <span>{bookedData.seats.map((x)=> x+" ")}</span>
          <br></br>
          
          <span>{todays}</span>
          <br></br>
          <span>{bookedData.time}</span>
